...
Since we haven't used Vim yet, we can use `ssh-keygen` to create a key on my `ieng6` computer, then adding key information to my `~/.ssh/config` file, then using the `scp` command to transfer it to my `ieng6` account. (As seen below.)

![scp commands for github keys](ieng6keynew.png)
...
![all 3 files copied to ieng6](iengkeysls.png)
...
![ssh key viewed on github account](githubsshkeys.png)

We know this works when we can use `git push origin main` on our `ieng6` account.
![using git push on ieng6 server](gitpushssh.png)
